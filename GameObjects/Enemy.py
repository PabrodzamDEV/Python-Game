...
class Enemy:
    ...
    def update(self, player, border, win_size: Tuple, dt):
        ...
        if (((
                ...
                and not (player.sprite.x + player.player_width // 2) >= self.sprite.x
                and (player.sprite.y + player.sprite.height // 3) >= self.sprite.y)
                and not (player.sprite.y + player.sprite.height // 3) > (self.sprite.y + self.sprite.height)):
            self.hit = True
